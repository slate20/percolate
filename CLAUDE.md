...
**Fixed-canvas backdrop + slot compositor**: `FarmScreen`'s landscape art is a single fixed 80x20 canvas (`data/farmhouse.json`'s `canvas` block) assembled at mount/resume by `backdrop_compositor.composite_backdrop` from named rectangular slots (`sky`/`house`/`shed`/`barn`, left-to-right per `docs/percolate_farm_tiers.md`), each an independent piece of ASCII art pasted into its own `(row, col, width, height)` region — see `docs/north_star.md` for why (an evolving, personalizable farm scene built from swappable pieces). Pieces are composited via `textual.content.Content` (not `rich.text.Text` — Rich's stricter color parser rejects bare names like `"gold"` that this art relies on; Textual's own parser accepts them), since Content tracks style spans independently of the text so pasting/slicing by column index can't misalign multiple pieces sharing a physical row. Each slot's `"art"` lines are **plain text, never inline markup**: color comes from a slot-level `"color"` field applied via `Content.stylize()`, not `Content.from_markup()` — ASCII art routinely contains literal `[`, `]` (window glyphs like `[]`, ground-line `][`) and trailing `\` characters that collide with markup's own `[tag]` syntax (bracket glyphs parse as bogus tags; a trailing `\` right before an appended `[/]` forms an accidental escape). If new art data still has inline `[color]...[/]` markup in it, that's a regression — convert it to plain text + a `"color"` field instead of trying to escape it.

Each slot holds a tiered `pieces` list (index = tier), and `backdrop_compositor.resolve_tiers(data, farm, upgrades_data)` works out each slot's current tier from player progression before compositing — still no player *choice* of piece, just automatic evolution (`north_star.md` Stage 1). A slot's `"progression"` key drives this: `{"upgrade": id}` reads `farm.upgrade_tier(id)` directly (`shed`/`plot_expansion`); `{"from_upgrades": [...], "combine": "rules", "rules": [...], "default": N}` builds a tier from more than one upgrade via ordered `{"if": {...}, "tier": N}` conditions, first match wins (`barn` needs both `roaster_slot` and `infuser` tiers); `{"derived_from": [slot_id, ...], "combine": "sum" | "rules", ...}` computes a tier from other *slots'* already-resolved tiers (`house` growing off `barn` + `shed`), resolved in a second pass so ordering in the JSON doesn't matter. Condition strings take an optional comparison prefix (`>=`, `<=`, `==`, `>`, `<`; bare number means `==`). Extend this rule table in JSON when adding new progression-linked art rather than hardcoding a new combine mode.

`#backdrop_wrap` is a non-focusable `ScrollableContainer` (mouse-wheel/scrollbar-drag panning only, never keybindings, so it can't shadow the plot-grid's arrow-key cursor) that auto-centers on the house slot on mount/resume. Nothing here polls on tick — the old `compact`/`large` discrete-variant switching (`FarmScreen._sync_backdrop`, `config.LARGE_BACKDROP_MIN_HEIGHT`) has been removed; don't reintroduce size-based variant polling for new backdrop-style content.
