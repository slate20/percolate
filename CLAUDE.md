...
**Size-adaptive backdrop**: `FarmScreen`'s landscape art (`data/farmhouse.json`) picks between `compact`/`large` variants by polling terminal size against `config.LARGE_BACKDROP_MIN_WIDTH/HEIGHT` on the existing tick — there's no dedicated resize-event plumbing, and ASCII art isn't meant to be dynamically rescaled, so new backdrop-style content should follow the same discrete-variant approach rather than trying to scale a single asset.
